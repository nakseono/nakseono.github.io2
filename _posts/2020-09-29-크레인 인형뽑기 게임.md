...
# 문제풀이
5개의 array. 후입선출. Stack 문제이다.  
만약 같은 모양의 인형이 바구니에 연속해서 쌓이면 두 인형은 바구니에서 사라진다.  
인형이 없을때는 크레인으로 뽑아도 아무일도 일어나지 않는다. (바구니에 공기가 들어가진 않는다.)  
바구니의 크기는 제약이 없다.  
격자상태가 담인 배열 board [[0,0,0,0,0],[0,0,1,0,3],[0,2,5,0,1],[4,2,4,4,2],[3,5,1,3,1]]  
